...
if [ "$#" -ne 7 ]; then
    echo "Usage: $0 [input_file] [output_directory] [genome_path] [bedtools_path] [cluster_merge_distance] [indel_ignore_threshold] [exclusion_mask]"
    exit 1
fi
echo HI
...
if [ "$EXCLUSION_MASK" != "" ]; then
    awk 'BEGIN {FS=OFS="\t"} ($3-$2 > 50000)' <($BEDTOOLS subtract -a "${OUTPUT_DIR}/list_cut_final.bed" -b "$EXCLUSION_MASK" -v) > "${OUTPUT_DIR}/list_cut_final_exclusion.bed"
