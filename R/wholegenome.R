...
scan_for_windows <- function(ref_fa, asm_fa, threads = 1, outfile = NULL, sizes = c(200000, 1000000, 5000000 ), ref_masked_regions = 'none') {
    ...
    for (i in 1:length(sizes)) {
        ...
        size <- sizes[i]
        wga_write_interval_list(ref_fa, asm_fa, outdirectory, paste0(outdirectory, '/list_cut_final.bed'), size, 10000, ref_masked_regions, threads)
    ...
    # if windows_df has 0 rows, exit the program with an error
    if (nrow(windows_df) == 0) {
        stop('No windows found. Please check the input files and parameters.')
    }
