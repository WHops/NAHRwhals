...
scan_for_windows <- function(ref_fa, asm_fa, threads = 1, outfile = NULL, sizes = c(200000, 1000000, 5000000 ), ref_masked_regions = 'none') {
    ...
    for (i in 1:length(sizes)) {
        ...
        run_silent(paste0('cp ', outdirectory, '/list_cut_final.bed ', paste0(outdirectory, '/list_cut_final_', i, '.bed')))
        Sys.sleep(0.5)
        ...
        if (!is.null(cumulative_bed)) {
            ...
            run_silent(paste0(sys_command_2))
            Sys.sleep(0.5)
            # Now move the sorted results to the final file
            run_silent(paste0('mv ', paste0(outdirectory, '/temp_list_cut_final.bed '), paste0(outdirectory, '/res_list_cut_final.bed')))
            Sys.sleep(0.5)
    ...
    run_silent(paste0('mv ', cumulative_bed, ' ', outfile))
