...
filter_res <- function(res, threshold){
  ...
  if ((dim(res)[2]-1) == 1){
    ...
    res$n_mut = (dim(res)[2]-1) - (
                                  rowSums(
                                     replace(
                                              (res[,2:dim(res)[2]]  == 'ref'), 
                                              is.na((res[,2:dim(res)[2]]  == 'ref')), 
                                              T
                                             )
                                   )
                                   )
...
# filter_res(res, 98)
# 
# res = data.frame(eval=c(98.688, 98.688, 99, 99.1), mut1=c('ref','ref', 'inv', 'del'))
# filter_res(res, 98)
# 
# 
# res = data.frame(eval=c(99.1, 99.5, 94.4, 97, 97.0), mut1=c('ref','ref', 'inv', 'del', 'dup'), mut2=c(NA,NA,'inv',NA, NA))
# filter_res(res, 98)
# 
